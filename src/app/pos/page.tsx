...
export default function POSSayfasi() {
  ...
  return (
    <div className="flex gap-0 h-[calc(100vh-48px)] -mx-6 -mt-6">
      ...
      {/* Sağ — İki Ayrı Panel */}
      <div className="w-[520px] flex flex-col gap-0 bg-slate-100 border-l border-slate-200 overflow-hidden">

        {/* ┌─────────────────────────────┐
            │  KUTU 1: SEPET (ürünler)   │
            └─────────────────────────────┘ */}
        <div className="flex flex-col bg-white m-3 mb-1.5 rounded-2xl border border-slate-200 shadow-sm overflow-hidden" style={{minHeight: 0, flex: "1 1 0"}}>
          {/* Başlık */}
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-900 rounded-t-2xl">
                    ...
                    <div className="flex items-center justify-between px-3 pb-3 gap-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => adetAyarla(k.urun.id, -1)}
                          className="w-8 h-8 bg-slate-100 hover:bg-slate-200 rounded-lg flex items-center justify-center transition-colors text-slate-700">
                          <Minus size={14} />
                        </button>
                        <input type="number" min={1} max={k.urun.stok} value={k.adet}
                          onChange={(e) => adetDirektGir(k.urun.id, e.target.value)}
                          className="w-12 text-center text-base font-black text-slate-900 border-2 border-slate-200 rounded-lg py-1 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
        ...
        {/* ┌─────────────────────────────┐
            │  KUTU 2: ÖDEME ALANI       │
            └─────────────────────────────┘ */}
        <div className="bg-white m-3 mt-1.5 rounded-2xl border border-slate-200 shadow-sm overflow-hidden shrink-0">
          {/* Başlık */}
          <div className="px-4 py-3 border-b border-slate-100 bg-indigo-600 rounded-t-2xl">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-indigo-200 uppercase tracking-widest">Ödeme</span>
              <span className="text-xl font-black text-white">{fmt(toplamTutar)}</span>
            </div>
            {/* Ara toplam + indirim özet */}
            <div className="flex items-center gap-3 mt-2">
              <span className="text-xs text-indigo-300">Ara: {fmt(araToplam)}</span>
              <div className="flex items-center gap-1 bg-indigo-500 rounded-lg px-2 py-0.5">
                ...
                <input type="number" min="0" max={araToplam} value={indirim}
                  ...
                  className="w-16 text-xs font-bold bg-transparent text-white placeholder-indigo-300 focus:outline-none"
                  placeholder="İndirim ₺" />
              </div>
              {indirimTutar > 0 && (
                <span className="text-xs text-emerald-300 font-bold">-{fmt(indirimTutar)}</span>
              )}
            </div>
          </div>
          ...
          <div className="p-4 space-y-3">
            ...
            {odemeTuru === "NAKIT" && (
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <input type="number" min={0} value={alinanPara}
                    onChange={(e) => setAlinanPara(e.target.value)}
                    className="flex-1 px-3 py-2 border border-amber-200 rounded-lg text-sm font-bold text-right bg-white focus:outline-none focus:ring-2 focus:ring-amber-400"
                    placeholder={toplamTutar.toFixed(2)} />
                </div>
                <div className="flex gap-1.5 flex-wrap">
                  {[50, 100, 200, 500].map((t) => (
                    <button key={t} onClick={() => setAlinanPara(t.toString())}
                      className="px-3 py-1 bg-white border border-amber-200 rounded-lg text-xs font-bold text-amber-700 hover:bg-amber-100 transition-colors">
                      ₺{t}
                    </button>
                  ))}
                  <button onClick={() => setAlinanPara(toplamTutar.toFixed(2))}
                    className="px-3 py-1 bg-amber-400 hover:bg-amber-500 rounded-lg text-xs font-bold text-white transition-colors">
                    Tam
                  </button>
                </div>
                {paraUstu > 0 && (
            ...
            {odemeTuru === "KART" && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex items-center gap-3">
                <CreditCard size={22} className="text-blue-500 shrink-0" />
                <div>
                  <p className="text-xs text-slate-400">Kart ile tahsil edilecek</p>
                  <p className="text-lg font-black text-slate-900">{fmt(toplamTutar)}</p>
                </div>
              </div>
            )}
              ...
              <div className="bg-purple-50 border border-purple-100 rounded-xl p-3 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <input type="number" min={0} value={pNakit}
                    ...
                    className="flex-1 px-3 py-1.5 border border-purple-200 rounded-lg text-sm font-bold text-right bg-white focus:outline-none focus:ring-2 focus:ring-amber-400"
                    placeholder="0.00" />
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <input type="number" min={0} value={pKart}
                    ...
                    className="flex-1 px-3 py-1.5 border border-purple-200 rounded-lg text-sm font-bold text-right bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                    placeholder="0.00" />
                </div>
                <div className="flex gap-1.5">
                  {[25, 50, 75].map((y) => (
                    <button key={y} onClick={() => { const n = (toplamTutar * y) / 100; setPNakit(n.toFixed(2)); setPKart((toplamTutar - n).toFixed(2)); }}
                      className="flex-1 py-1 bg-white border border-purple-200 rounded-lg text-xs font-bold text-purple-700 hover:bg-purple-100 transition-colors">
                      %{y} Nakit
                    </button>
                  ))}
                </div>
                <div className={`flex justify-between text-sm font-bold rounded-lg px-3 py-2 border ${parcaliEksik ? "bg-red-50 border-red-100 text-red-600" : "bg-emerald-50 border-emerald-100 text-emerald-700"}`}>
            ...
            <button
              ...
              className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-xl font-black text-base transition-colors shadow"
            >
